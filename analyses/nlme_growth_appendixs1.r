...
### Which fitting routine to use? nls(), gnls(), nlsList(), or nlme()? If you
#have no treatment effects, ie, a model of the form y ~ x, use gnls. It can do
#everything that nls can do, but additionally allows variance modelling to deal
#with heteroscedasticity. Also, gnls allows you to choose which parameters
#should vary among treatment groups, and which should be global. See Pinhero &
#Bates, pg 401, for an example. However, occasionally gnls will fail to
#converge. In those cases, nls can still be useful. Use nlsList to fit a series
#of nls models (this is rarely useful in practice. Rather, you probably want to
#use nlme) nlme can do all of the above, as well as allowing for fixed and
#(nested) random effects. However, it requires a somewhat different syntax for
#specifying the model. See Pinheiro, J.C., and Bates, D.M. (2000) "Mixed-Effects
#Models in S and S-PLUS", Springer for much more detail on these topics. ##
...
### measuring RGRs         ###
##############################
### Primary task illustrated in this figure: propagating error from parameters
#into estimates of RGR. Before propagating error, we need to check that the
#parameter profiles are approximately V-shaped (and that the sampling intervals
#of the parameters are therefore approximately multivariate normal) before
#proceeding. To do this, we need to fit a seperate nls fit for each species. We
#have to use nls(), rather than gnls() or nlsList, because profile methods do
#not exist for those functions.
