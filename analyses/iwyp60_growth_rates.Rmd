...
Which fitting routine to use [nls, _gnls_, nlsList, or nlme]? If there are no treatment effects (ie y ~ x) use gnls. It can do everything that nls can do, but additionally allows variance modelling to deal with heteroscedasticity. Also, gnls allows you to choose which parameters should vary among treatment groups, and which should be global (Pinhero & Bates, pg 401). However, occasionally gnls will fail to converge. In those cases, nls can still be useful. Use nlsList to fit a series of nls models (this is rarely useful in practice. Rather, you probably want to use nlme) nlme can do all of the above, as well as allowing for fixed and (nested) random effects. However, it requires a somewhat different syntax for specifying the model (Pinheiro, J.C., and Bates, D.M. 2000).
...
```{r, results='markup'}
#fit
fit.mono <- gnls(Y ~ SSasymp(X_num, Asym, R0, lrc), data = df)
yfitted <- predict(fit.mono, list(X_num = xNew))

#plot
plot(Y ~ X, df, ylab = "Growth (NDVI)", xlab = "Date")
lines(xNew, yfitted, col = 'red') 
mtext("Monomolecular model", font=3, padj = -1)
```
...
```{r, results='markup'}
#fit
fit.logis  <- gnls( Y ~ SSlogis(X_num, Asym, xmid, scal), data = df)
yfitted <- predict(fit.logis, list(X_num = xNew))
#with variance function
fit.logis.var <- gnls(Y ~ SSlogis(X_num, Asym, xmid, scal), data = df, weights= varExp(form = ~ fitted(.)))
yfitted2 <- predict(fit.logis.var, list(X_num = xNew))

#plot
plot(Y ~ X, df, ylab = "Growth (NDVI)", xlab = "Date")
lines(xNew, yfitted, col = 'red')
lines(xNew, yfitted2, col = 'blue') 
mtext("3-P Logistic models", font=3, padj = -1)
```

### Comparison

Compare measures of model fit and residual plots for each model produced

```{r, echo=FALSE, results='markup'}
anova(fit.mono, fit.logis, fit.logis.var)
plot(fit.mono, main='Monomolecular')
plot(fit.logis, main='Logistic')
plot(fit.logis.var, main='Logistic with variance function')
```
