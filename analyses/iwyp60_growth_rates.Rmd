...
for(i in unique(interaction(df$group, df$dataset))){
  a <- tibble(Y=0, X=as.Date("2018-07-01"), 
              logY=log10(0.01),
...
out <- rbind(c("3P ExK", fit.logis.exk$logLik),
             c("3P Extras", fit.logis.ext$logLik),
             c("4P ExK", fit.fpl.exk$logLik),
             c("4P Extras", fit.fpl.ext$logLik)
)
...
So the 3P logistic may be the "best fit" 

Aim: We want to determine AGR, RGRm, RGRt *per plot* (ie. using plot IDs = "_IW_P_XYZ"). For this, we will need *nlme* to converge so we can assign plot ID as a fixed effect and also include spatial info as random (TBD). 
