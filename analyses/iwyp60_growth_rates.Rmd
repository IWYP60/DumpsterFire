...
*N.B. Gompertz models would not converge and 4P had limited convergence*
...
df <- dat %>%
  ...
  filter(X != "2018-11-16") %>% # remove last data point to help convergence
...
## estimate initial coefs with "getInitial" helps gnls to converge
...
Having M0 = 0 is problematic and messes up the maths for obtaining growth rates. 4P logistic does *not* appear to result in M0 = 0. So, we could probably obtain some growth rate estimates with that.
...
> Profiles are v-shaped (also for 3P; not shown above).

#### Extract AGR and RGR using 4P fit
...
#### Take random ID and plot all data together (NDVI, RGR, AGR)
...
for(i in sample(1:length(unique(rates$id)), size = 2, replace = FALSE)){
  tryCatch({
      ...
      # "biomass"
      plot(Y ~ X, data = a, ylab = "NDVI", xlab = "", ylim = c(0,.8))
      lines(xNew, yfitted, col = 'red')
      mtext(paste(Id), font=3, padj = -1)
      ...
      # AGR
      plot(AGR ~ times, data=b, type = "n", xlab='', ylab = expression(paste("AGR ", (unit^-1%.%day^-1))))
      lines(AGR ~ times, data = b)
      
      # RGRt
      plot(RGRt ~ times, data=b, type = "n", xlab='', ylab = expression(paste("RGR ", (g%.%g^-1%.%day^-1))))
      lines(RGRt ~ times, data = b)
      
      # RGRm
      plot(RGRm ~ times, data=b, type = "n", xlab='Days', ylab = expression(paste("RGR ", (unit%.%unit^-1%.%day^-1))))
      lines(RGRm ~ times, data = b)
  }, 
  error = function(e){cat("ERROR @ #", i, ":", conditionMessage(e), "\n")})
